...
def rtk_crc24q(buff, length):
  #  print("CRC MODULE")
  #  print("type",type(buff))
  #  print("buff=",buff)
  #  print("bufflength=",length)
    """Build the CRC Code
  ...
  #  print("buf after zero=",buff)
  # print("len after zero=",length)
...
class NavicDataGen():
    ...
    def GenerateBits(self, timeInterval):
      ...
      #print("navic data gen",genStream[1:numBitsToGen+1])
...
def navic_pcps_acquisition(x, prnSeq_pilot, fs, fSearch, threshold=0):
    ...
    print("pilotSig=\n ",pilotSig[:20])
    #print("pilotSig_len=\n",len(pilotSig))
    pilotXsubc=pilotSig*subCarrier
    with open('subcarrier.dat','w') as f:
        for i in subCarrier:
            f.write( str(i) + '\n')
    with open('pilotsig.dat','w') as f:
        for i in pilotSig:
            f.write(str(i)+'\n')
    with open('pilotXsubc.dat','w') as f:
        for i in pilotXsubc:
            f.write(str(i)+'\n')
    
    
    print("pilotXsubc=\n",(pilotSig*subCarrier)[:20])
    print("before cconjugate=\n",(np.fft.fft(pilotSig*subCarrier))[:20])
    prnSeq_pilot_FFT = np.conjugate(np.fft.fft(pilotSig*subCarrier))
    with open('prnfft.dat','w') as f:
        for i in prnSeq_pilot_FFT:
            f.write(str(i)+'\n')

    print("pnrSeq_pilot_FFT=\n",prnSeq_pilot_FFT[:20])
...
fShift = np.array([3801,3921,3919,4961])
...
#hihi,bibi=datagen.SymbolsGen()
#print(bibi.shape)
#navdata=datagen.GetDataStream() #Unencrypted data
#print("DATA SHAPE:",navdata.shape)
#print(navdata)
for i in range(satVis):
    ...
    print("satellite=",i)
    num_sf = len(bits)//600
    #navbits = datagen.GetSymbolStream()[0:600,0].reshape(-1,600)
    ...
    if(navbits[lckdidx,i]==bits[lckdidx]):
...
# Compare Subframe1 data
subframe1_before_encoding = navdata[:9, i]
subframe1_after_decoding = s1_decoded

# Compare Subframe1 data after encoding and receiving
subframe1_after_encoding = navbits[:52, i]
subframe1_after_receiving = bits[:52]

# Compare Subframe2 data before and after decoding
subframe2_before_encoding = navdata[9:609, i]
subframe2_after_decoding = s2_decoded
# Compare Subframe2 data after encoding and receiving
subframe2_after_encoding = navbits[52:1252, i]
subframe2_after_receiving = bits[52:1252]

# Compare Subframe3 data before and after decoding
subframe3_before_encoding = navdata[609:, i]
subframe3_after_decoding = s2_decoded

# Compare Subframe3 data after encoding and receiving
subframe3_after_encoding = navbits[1252:, i]
subframe3_after_receiving = bits[1252:]

# Check if the data is equal and print the results
def print_comparison_result(name1, data1, name2, data2):
    if np.array_equal(data1, data2):
        print(f"{name1} is equal to {name2}.")
    else:
        print(f"{name1} is not equal to {name2}.")

print_comparison_result("Subframe1 Before Encoding 9 bits", subframe1_before_encoding, "Subframe1 After decoding 9 bits", subframe1_after_decoding)
print_comparison_result("Subframe2 Before Encoding 600 bits", subframe2_before_encoding, "Subframe2 After decoding 600 bits", subframe2_after_decoding)
print_comparison_result("Subframe2 After Encoding 1200 Symbols", subframe2_after_encoding, "Subframe2 After Receiving 1200 Symbols", subframe2_after_receiving)
print_comparison_result("Subframe3 Before Encoding 274 bits", subframe3_before_encoding, "Subframe3 After decoding 274 bits", subframe3_after_decoding)
print_comparison_result("Subframe3 After Encoding 548 Symbols", subframe3_after_encoding, "Subframe3 After Receiving 548 Symbols", subframe3_after_receiving)

# Print the sizes
print("Size of Subframe3 Before Encoding 274 bits:", len(subframe3_before_encoding))
print("Size of Subframe3 After Decoding 274 bits:", len(subframe3_after_decoding))
print("Size of Subframe3 After Encoding 548 Symbols:", len(subframe3_after_encoding))
print("Size of Subframe3 After Receiving 548 Symbols:", len(subframe3_after_receiving))
...
data_sent = datagen.GetSymbolStream()[:-1, 0]
