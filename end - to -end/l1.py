...
def genNavicCaTable_Pilot(samplingFreq,codeLength, codeFreqBasis, satId):
    ...
    indexArr = indexArr.astype(int)
    temp = []
    temp = np.array([genNavicCaCode_Pilot(i) for i in satId])[:,indexArr].T
    np.savetxt("p_up.txt",temp)
...
class NavicDataGen():
    ...
    def SymbolsGen(self):
        ...
        subframe1_data = np.flip(subframe1_data)
...
def navic_pcps_acquisition(x, prnSeq_pilot, prnSeq_data, fs, fSearch,coherent_integ, non_coherent_integ, threshold=10):
    ...
    for i in range(0,N):
        ...
        for  a in range(0,non_coherent_integ):
            ...
            for b in range(0, coherent_integ):
                start_p = b*K
                end_p = (b*K)+(K)
                x_iq_p = x[start_p:end_p]*np.exp(-1j*2*np.pi*(fSearch[i]+if_frequency)*t)
...
class NavicTracker:
    ...
    def updatePromptCode(self):
       ...
       # codeNumSample_pilot = self.codeTable_pilot.shape[0]
        ...
        t = np.arange(0, 1, 1/(self.SampleRate * self.PLLIntegrationTime*1e-3))
...
def samp_read(sample_count, file):   
    ...
    waveform_for_acq = f_interp(t_new)
    return waveform_for_acq
